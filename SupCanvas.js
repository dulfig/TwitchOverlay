...
img.onload = start;
img.src = "https://i.imgur.com/wQat8X4.png";
canvas.width = 1906;
canvas.height= 108;
...
function Circle(x,y,dx,dy){
    ...
    this.update=function() {
        if (this.x > 1888 || this.x < 24)
            this.dx = -this.dx;
        if (this.y > 89 || this.y < 24)
            this.dy = -this.dy;
        this.x += this.dx;
        this.y += this.dy;
        this.draw();
...
for(var i=0;i<amount;i++) {
    var x = getRandomInt(24,1888 );
    var y = getRandomInt(24, 89);
